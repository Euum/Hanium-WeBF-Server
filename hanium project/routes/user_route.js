    ...
    sessionCheck(req,
        function () {
            req.session.destroy(
                function (err) {
                    if (err) {
                        res.send("<script>alert('" + err + "'); window.location = '/';</script>");
                        return;
                    } else {
                        res.redirect('/');
                    }
        ...
        function () {
            res.send("<script>alert('이미 로그아웃 되었습니다.'); window.location = '/';</script>");
        }
    );
};
        ...
        addUser(db, id, password, email, (err, user) => {
            try {
                ...
                if (user) {
                    ...
                    res.write(
                        `
                        <script>alert('회원가입 완료'); window.location.href='/';</script>
                        `
                    );
                    res.end();
                ...
                res.writeHead(200, { "Content-type": "text/html;charset=utf8" });
                res.write("<script>alert('ERROR: '"+e.stack+"'); window.location.href='/';</script>");
                res.end();
